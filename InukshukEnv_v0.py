...
class InukshukEnv_v0(gym.Env):
    def __init__(self, render: bool = False):
        ...
        self.observation_space = spaces.flatten_space(self.unflatten_observation_space)
    ...
    def __get_observation(self):
        state = np.zeros((10, 7), dtype=np.float64)
        if not len(self.stone_container):
            return spaces.flatten(self.unflatten_observation_space, state)
        else:
    ...
    def step(self, action):
        ...
        state_flat = self.__get_observation()
        state = spaces.unflatten(self.unflatten_observation_space, state_flat)
        self.step_num += 1
        ...
        if self.collision is True or len(self.stone_container) >= 10:
            done = True
        else:
            done = False
        info = {}
        return state_flat, reward, done, info
