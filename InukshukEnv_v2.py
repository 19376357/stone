...
class InukshukEnv_v2(gym.Env):
    def __init__(self, render: bool = False):

        self._render = render
        # 定义动作空间
        self.action_space = spaces.MultiDiscrete(np.array([11, 6, 2]))
        ...
        self.index_size = {0: '1_2_2', 1: '2_2_2', 2: '3_2_2', 3: '4_2_2', 4: '5_2_2', 5: '6_2_2',
                           6: '7_2_2', 7: '8_2_2', 8: '9_2_2', 9: '10_2_2'}
    ...
    def __apply_action(self, action):
        ...
        index = action[0]
        if index:
            #self.state_x.append(action[2])
            if self.step_num % 6 == 1 or self.step_num % 6 == 2:
                self.state_y.append(action[2])
                self.state_x.append(action[1])
            #self.state_z.append(action[4])
            ...
            position = np.array([0, action[1] * 0.2, height + 1.])
            orientation = p.getQuaternionFromEuler(
                np.array([0, self.h[self.step_num % 6] * np.pi / 2, np.pi / 2])
            )
    ...
    def step(self, action):
            ...
            if len(contact_list) == 1:
                ...
                for item in contact_list:
                    if item[0] == 0:
                        ...
                        reward_contact += 0.1 * (12-len(self.stone_container))
        ...
        if self.collision is True or len(self.stone_container) >= 20 or len(contact_list) > 2:
            reward_similarity = 0
            reward_holes = 0
            reward_x = 0
            reward_y = 0
